...
export default function EditPortfolioPage() {
  ...
  const fetchPortfolio = async () => {
    try {
      setIsFetching(true);
      const response = await adminPortfolioApi.getById(portfolioId);
      if (response.success && response.portfolio) {
        ...
        setExistingImage(portfolio.imageUrl || '');
      } else {
        setError('Portfolio item not found');
      }
    } catch (error) {
      console.error('Error fetching portfolio:', error);
      setError(error.response?.data?.message || 'Failed to load portfolio item');
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await adminPortfolioApi.update(portfolioId, payload);
      ...
      setError(error.response?.data?.message || 'Failed to update portfolio item');
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Update portfolio item for Performance-Driven Designs section</p>
      </div>
