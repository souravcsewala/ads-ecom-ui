...
export default function EditTeamMemberPage() {
  ...
  const fetchMember = async () => {
    try {
      setIsFetching(true);
      const response = await adminTeamApi.getById(memberId);
      if (response.success && response.teamMember) {
        ...
        if (member.image?.url) {
          setExistingImage(member.image.url);
        }
      } else {
      ...
      setError(error.response?.data?.message || 'Failed to load team member');
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Only append image file if a new file is selected
      if (selectedFile) {
      ...
      const response = await adminTeamApi.update(memberId, payload);
      ...
      setError(error.response?.data?.message || 'Failed to update team member');
      ...
      <div className="bg-white rounded-lg shadow-md p-8">
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Image {existingImage ? '(Leave empty to keep current)' : '*'}
              </label>
              {existingImage && !preview && (
                <div className="mb-4">
                  <p className="text-sm text-gray-600 mb-2">Current Image:</p>
                  <img src={existingImage} alt="Current" className="w-32 h-32 rounded-full object-cover border-2 border-gray-300" />
                </div>
              )}
              <input
                type="file"
